...
npz_path = "/scratch/st-tklee-1/ndsz/data/multispace_npz"
...
def augment(image, ground_truth, images, ground_truths, embeddings, hsv_images, hsv_embeddings):
    num_img = 0
    remixes_per_iter = 6
        ...
        try:
            ...
            if (np.count_nonzero(gt) > 100):
                ...
                with torch.no_grad():
                    ...
                    hsv_embeddings.append(hsv_embedding.cpu().numpy()[0])
...
def process(gt_name: str, image_name: str):
    ...
    if (np.sum(gt_data) > 100):
        ...
        images, ground_truths, embeddings = augment(ori_image_data, ori_gt_data, images, ground_truths, embeddings, hsv_images, hsv_embeddings)

        save_im_gt_emb_as_npz(fname, images, ground_truths, embeddings)
        save_im_gt_emb_as_npz(f"hsv_{fname}", images, ground_truths, embeddings)
    return
